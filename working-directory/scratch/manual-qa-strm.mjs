...
if (!mapPath || !srcPath || !tgtPath || !reviewLogPath || !label) {
  console.error('Usage: node manual-qa-strm.mjs <mapPath> <srcCsv> <tgtCsv> <reviewLogPath> <label>');
  process.exit(1);
}
...
function toks(t) {
  return clean(t).toLowerCase().replace(/[^a-z0-9\s-]/g, ' ').split(/\s+/).filter((x) => x.length >= 3 && !STOP.has(x));
}
function jac(a, b) {
...
function hasStrong(text) {
  return /\b(shall|must|required|ensure|implement|establish)\b/i.test(text);
}
function hasWeak(text) {
  return /\b(should|may|recommended|as applicable|consider)\b/i.test(text);
}
...
async function parseById(path) {
  ...
  for (const r of rows.slice(1)) {
    ...
    m.set(id, {
      ...
      title,
      description,
      titleTokens: new Set(toks(title)),
      descTokens: new Set(toks(description)),
...
for (let i = 1; i < rows.length; i += 1) {
  ...
  if (rel === 'equal') {
    if (titleJ < 0.2 || (srcStrong && tgtWeak) || (srcWeak && tgtStrong) || descJ < 0.05) {
      ...
      reason.push('downgraded equal after manual check: insufficient title/description parity or modal mismatch');
    }
  } else if (rel === 'intersects_with') {
    if (titleJ >= 0.34 && descJ >= 0.12 && !((srcStrong && tgtWeak) || (srcWeak && tgtStrong))) {
      nextRel = 'equal';
      reason.push('promoted intersects_with to equal after manual parity check');
    }
  }

  if (rel === 'subset_of' && srcStrong && tgtStrong && descJ > 0.2 && titleJ > 0.22) {
    nextRel = 'equal';
    reason.push('promoted subset_of to equal after manual parity check');
  }
  if (rel === 'superset_of' && srcStrong && tgtStrong && descJ > 0.2 && titleJ > 0.22) {
    nextRel = 'equal';
    reason.push('promoted superset_of to equal after manual parity check');
  }
  ...
  if (nextRel !== rel) {
    ...
    r[idx['Notes']] = prevNotes ? `${prevNotes} Manual QA: ${reason.join('; ')}.` : `Manual QA: ${reason.join('; ')}.`;
    changed.push({ row: i + 1, fde, target: tid, from: rel, to: nextRel });
  }
}

await fs.writeFile(mapPath, toCsv(rows), 'utf8');

const byType = changed.reduce((a, c) => {
  const k = `${c.from}->${c.to}`;
  a[k] = (a[k] ?? 0) + 1;
  return a;
}, {});
...
const log = [
  ...
  '',
  '## Changed FDE Rows',
  ...changed.map((c) => `- Row ${c.row}: ${c.fde} -> ${c.target} (${c.from} => ${c.to})`),
  '',
].join('\n');
