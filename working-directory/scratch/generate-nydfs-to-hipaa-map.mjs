...
const sourcePath = 'working-directory/scratch/nydfs-extracted.csv';
const targetPath = 'working-directory/scratch/hipaa-extracted.csv';
const outputPath = 'working-directory/mapping-artifacts/2026-03-24_NYDFS-to-HIPAA/Set Theory Relationship Mapping (STRM)_ [(NYDFS-to-NYDFS)-to-HIPAA] - NYDFS to HIPAA.csv';
...
const THEMES = [
  ...
  { name: 'data protection', words: ['confidentiality','integrity','availability','encrypt','nonpublic','protect'] },
...
function hasWeak(text) {
  return /\b(should|may|consider|as applicable)\b/i.test(text);
}
...
function relationTail(rel) {
  ...
  if (rel === 'subset_of') return 'NYDFS is narrower and fits within the HIPAA requirement scope.';
  if (rel === 'superset_of') return 'NYDFS covers broader scope than the mapped HIPAA requirement.';
...
const outRows = [buildHeader('HIPAA')];
...
for (const s of sources) {
  ...
  const rationale = `NYDFS ${s.controlId} requires ${clip(s.description, 180)}. HIPAA ${t.controlId} requires ${clip(t.description, 180)}. Both address ${sharedTheme}. ${relationTail(relationship)}`;
